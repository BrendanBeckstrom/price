#!/usr/bin/env python3
"""Load per-run constraint_*.npy under results/task_k, compute IoU vs wall ground truth, plot heatmaps."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare task_k constraint grids vs UMaze wall mask.")
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=None,
        help="Directory containing one subfolder per run (default: ./results/task_k next to this script).",
    )
    parser.add_argument(
        "--ground-truth",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Figure output path (default: task_k_comparison.pdf next to this script).",
    )
    parser.add_argument(
        "--csv",
        type=Path,
    ...
    results_dir = args.results_dir or (script_dir / "results" / "task_k")
    gt_path = args.ground_truth or (script_dir / "ground_truth_maze.npy")
    out_path = args.output or (script_dir / "task_k_comparison.pdf")
    ...
    if not grids:
        raise SystemExit(f"No constraint_{args.constraint_epoch}.npy found under {results_dir}")
