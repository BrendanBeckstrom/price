@echo off
REM PRICE-forward v2 (K=10): all five conditions (clean, vr5, vr10, vr20, vr50).
...
set "RESULT_ROOT=experiments\breaking\results\price_forward_suboptimal_v2"
set "COMMON=--icl_config.task AntMaze_UMazeDense-v3 --icl_config.constraint_type Maze --icl_config.seed 100 --icl_config.suffix breaking_subopt_price_forward_v2 --exp_name breaking_subopt_price_forward_v2 --maze_task -1 --epochs 5 --use_price true --icl_config.expert_batch_fraction 0.25 --icl_config.price.route_all_dispreferred true --icl_config.price.routed_batch_fraction 1.0 --icl_config.price.pairs_per_constraint_phase 256 --icl_config.price.routed_positions_cap 16384"
