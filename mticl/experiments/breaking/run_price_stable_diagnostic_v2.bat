@echo off
REM PRICE stability diagnostic v2: vr20 only, frozen-growing routed pool.
REM routed_batch_fraction=1.0 (back up from 0.5), constraint_steps=2500.
REM Verify the strictly-growing pool breaks per-epoch violation oscillation.
...
set "RESULT_ROOT=experiments\breaking\results\price_stable_diagnostic_v2"
set "OUTDIR=%RESULT_ROOT%\violate_20"
if not exist "%OUTDIR%" mkdir "%OUTDIR%"

set "COMMON=--icl_config.task AntMaze_UMazeDense-v3 --icl_config.constraint_type Maze --icl_config.seed 100 --icl_config.suffix breaking_price_stable_diag_v2 --exp_name breaking_price_stable_diag_v2 --maze_task -1 --epochs 5 --use_price true --icl_config.expert_batch_fraction 0.25 --icl_config.constraint_steps 2500 --icl_config.price.route_all_dispreferred true --icl_config.price.routed_batch_fraction 1.0 --icl_config.price.pairs_per_constraint_phase 256 --icl_config.price.routed_positions_cap 16384 --demo_dir demos/corrupted --demo_suffix _vr20 --icl_config.log_path %OUTDIR%"

echo.
echo =================================================================
echo   PRICE Stability Diagnostic v2 -- vr20 (frozen-growing pool)
echo =================================================================
